...
void setup() 
{
  ...
  verticalColor = new color[background.width]; // Empty array to hold colors
...
void draw() 
{
  ...
  //image(background, 0, 0); // Unnecessary after adding the trail function
  float x = 750+sin(time)*750; // X coordinate for the sprite after scaling
  float y = 1000 +cos(time)*200 ; // Y coordinate for the sprite after scaling
...
void LeaveTrail(PImage src, PImage out, float xin, float yin)
{
  src.loadPixels();
  out.loadPixels();
  for (int x = 0; x < src.width - 850; x ++ ) {
    for (int y = 380; y < src.height - 480; y ++ ) {
      ...
      } else out.pixels[index] = src.pixels[index]; // If we are not in the area we want to be, leave the background as is
    }
  }
